...
import { SectionHeader, Placeholder, MarketChip, OtaChip, XLink } from '../components/ui/index.js';

// Slug registry — add new hub here, nothing else to touch
const HUB_REGISTRY = {
  'halong-hub':  { page: lazy(() => import('./halong/index.jsx')),   prefix: 'hl-' },
  'hanoi-hub':   { page: lazy(() => import('./hanoi/index.jsx')),    prefix: 'hn-' },
  'hagiang-hub': { page: lazy(() => import('./hagiang.jsx')),  prefix: 'hg-' },
  'sapa-hub':    { page: lazy(() => import('./sapa.jsx')),     prefix: 'sp-' },
  'ninhbinh-hub':{ page: lazy(() => import('./ninhbinh.jsx')), prefix: 'nb-' },
};

function HubSuspense({ children }) {
  return <Suspense fallback={<div style={{ padding: 48, textAlign: 'center', color: 'var(--ink-3)' }}>Đang tải…</div>}>{children}</Suspense>;
}
...
function ToursPage({ route, go }) {
  ...
  // Hub overview
  if (id && HUB_REGISTRY[id]) {
    const { page: Page } = HUB_REGISTRY[id];
    return <HubSuspense><Page go={go} /></HubSuspense>;
  }

  // Variant detail — find hub by prefix
  if (id) {
    const entry = Object.entries(HUB_REGISTRY).find(([, { prefix }]) => id.startsWith(prefix));
    if (entry) {
      const { page: Page } = entry[1];
      return <HubSuspense><Page id={id} go={go} variant /></HubSuspense>;
    }
    return <TourDetail id={id} go={go} />;
  }
        ...
        <div className="stage">
          <div style={{ display: 'flex', gap: 32, alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              ...
              {regions.map((r) => <React.Fragment key={r}>{chipBtn(filterRegion === r, () => setFilterRegion(r), r === 'all' ? 'Tất cả' : r)}</React.Fragment>)}
            </div>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <span className="label" style={{ marginRight: 8 }}>Khách</span>
              {markets.map((m) => <React.Fragment key={m.id}>{chipBtn(filterMarket === m.id, () => setFilterMarket(m.id), m.flag + ' ' + (m.id === 'all' ? 'Tất cả' : m.name))}</React.Fragment>)}
            </div>
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 4, alignItems: 'center' }}>
              <span className="label" style={{ marginRight: 8 }}>Sắp xếp</span>
              {[['bookings','Booking T7'],['rating','Rating'],['price','Giá']].map(([k,l]) => <React.Fragment key={k}>{chipBtn(sortBy===k, ()=>setSortBy(k), l)}</React.Fragment>)}
            </div>
          </div>
