...
function Masthead({ route, go }) {
  const tabs = [
    ...
    { id: 'tours', label: 'Tour', count: window.DATA.tours.length },
    { id: 'markets', label: 'Khách', count: window.DATA.markets.length },
    { id: 'otas', label: 'Sàn OTA', count: window.DATA.otas.length },
  ];
  return (
...
function MonthChip({ id, go }) {
  const m = window.DATA.months.find((x) => x.id === id);
  if (!m) return null;
