// Generic destination hub template — used by hagiang, sapa, ninhbinh (and any future hub).
// Halong + Hanoi keep their own pages due to cruise-specific UI / SondaxPlaybookSection.
import React from 'react';
import { SectionHeader } from '../ui/index.js';
import { SubVariantsSection } from '../../pages/hanoi/index.jsx';

// ─── Hub overview page ────────────────────────────────────────────────────────

export function DestinationHubPage({ hub: H, go }) {
  ...
  return (
    <div className="page-anim">
      ...
      <section className="section" style={{ paddingBlock: '72px 48px' }}>
        <div className="stage">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 24 }}>
            <span className="folio" style={{ color: 'var(--accent)' }}>DESTINATION HUB · {H.name.toUpperCase()}</span>
            <span className="folio">Cập nhật 04/2026</span>
          </div>
          <div className="h-kicker">{H.subtitle}</div>
          <h1 className="h-display" style={{ fontSize: 80, lineHeight: 1, marginBottom: 24 }}>
            Thị trường<br />
            <em style={{ color: 'var(--accent)', fontStyle: 'italic', fontWeight: 300 }}>{H.market.bookingVolumeTotalEst || H.market.gmv2025}.</em>
          </h1>
          <p className="dek" style={{ fontSize: 22, maxWidth: 780 }}>{H.heroDescription}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 0, marginTop: 40, borderTop: '2px solid var(--ink)' }}>
            {[
              { label: 'Booking/năm',   val: H.market.bookingVolumeTotalEst || H.market.gmv2025, sub: `tổng các variant · ${H.name}` },
          ...
          <div className="grid-2" style={{ gridTemplateColumns: '280px 1fr', gap: 64 }}>
            <div>
              ...
              <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  { label: 'Cao điểm (≥90)',   bg: 'var(--accent)' },
                  { label: 'Trung bình (60–89)', bg: 'var(--accent-soft)' },
                  { label: 'Thấp (<60)',        bg: 'var(--paper-3)' },
                ].map(({ label, bg }) => (
                  <div key={label} style={{ display: 'flex', gap: 10, alignItems: 'center', fontSize: 13 }}>
                    <span style={{ display: 'inline-block', width: 12, height: 12, background: bg }} />
                    <span>{label}</span>
                  </div>
                ))}
              </div>
      ...
      {/* Top markets + Top OTAs */}
      <section className="section">
        <div className="stage">
          <SectionHeader kicker="Tổng quan thị trường" title={`Ai đến ${H.name} — và book qua đâu.`} dek="Tổng hợp toàn tỉnh. Chi tiết từng sản phẩm bên dưới." />
      ...
      {flagship && (
        <section className="section" style={{ background: 'var(--paper-3)' }}>
          <div className="stage">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 24 }}>
              <span className="folio" style={{ color: 'var(--accent)' }}>SẢN PHẨM DẪN ĐẦU {H.name.toUpperCase()}</span>
              <span className="folio" style={{ color: 'var(--ink-3)' }}>#1 TRONG {H.variants.length} LOẠI</span>
            </div>
            <div className="grid-2" style={{ gridTemplateColumns: '1.3fr 1fr', gap: 64, borderTop: '2px solid var(--ink)', paddingTop: 32 }}>
              <div>
                ...
                {flagship.description && (
                  <p className="body" style={{ fontSize: 16, color: 'var(--ink-2)', marginBottom: 20, lineHeight: 1.55 }}>{flagship.description}</p>
                )}
                <button
                  onClick={() => go({ tab: 'tours', id: flagship.id })}
                  style={{ fontFamily: 'var(--mono)', fontSize: 11, padding: '10px 18px', background: 'var(--ink)', color: 'var(--paper)', border: 'none', cursor: 'pointer', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                  Xem chi tiết sản phẩm này →
                </button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, borderLeft: '1px solid var(--rule)' }}>
                {[
                  { label: 'Traffic',      val: flagship.traffic,  sub: flagship.trafficNote || `#1 ${H.name}` },
                  { label: 'Giá thị trường', val: flagship.price,  sub: flagship.segment },
                  { label: 'Thị phần',    val: flagship.share,    sub: 'segment' },
      ...
      <section className="section" style={{ background: 'var(--paper-2)' }}>
        <div className="stage">
          <SectionHeader
            kicker={`${H.variants.length} loại sản phẩm ${H.name}`}
            title={H.variantsTitle || 'Toàn bộ danh mục sản phẩm.'}
          ...
          {(() => {
            ...
            const rows = [...H.variants]
              .map((x) => ({ ...x, _growthNum: parseInt((x.growth || '0').replace(/[^0-9-]/g, '')) || 0 }))
            ...
            return (
              <div style={{ borderTop: '2px solid var(--ink)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '36px 1.4fr 150px 90px 110px 120px 0.9fr', gap: 18, padding: '14px 0', borderBottom: '1px solid var(--ink)' }}>
                  {['#', 'Sản phẩm', 'Booking/năm', 'Đà tăng', 'Thị phần', 'Time-to-mkt', 'Ghi chú'].map((h) => (
                  ...
                  return (
                    <div key={t.id} style={{ display: 'grid', gridTemplateColumns: '36px 1.4fr 150px 90px 110px 120px 0.9fr', gap: 18, padding: '22px 0', borderBottom: '1px solid var(--rule)', alignItems: 'center' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink-3)', marginTop: 4, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t.segment} · {t.price}</div>
                      </div>
                      <div>
                      ...
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 500 }}>{t.share || <span style={{ color: 'var(--ink-4)' }}>—</span>}</div>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.4 }}>{t.timeToMarket || <span style={{ color: 'var(--ink-4)' }}>—</span>}</div>
                      <div style={{ fontSize: 12, color: 'var(--ink-2)', lineHeight: 1.5 }}>
                        {t.emergingNote
                          ? t.emergingNote
                          : t.isFlagship
                            ? `Flagship segment. ${t.share || ''}`
                            : t.skeleton
                              ? 'Skeleton tier — thin volume. Monitor only.'
                              : t._growthNum >= 25 ? 'Đà tăng mạnh. Cửa sổ 12–18 tháng consolidate.'
                              : t._growthNum >= 10 ? 'Đà tăng ổn định. Market đủ lớn nếu differentiation.'
      ...
      {H.topCompetitors && (
        <section className="section" style={{ background: 'var(--paper-2)' }}>
          <div className="stage">
            <SectionHeader kicker={`Top ${H.topCompetitors.length} đối thủ cạnh tranh`} title={`Ai đang chạy ${H.name} market.`} />
            <div style={{ borderTop: '2px solid var(--ink)' }}>
                      ...
                      <div>
                        ...
                        {c.strengths.map((s, j) => (
                          <div key={j} style={{ display: 'flex', gap: 10, fontSize: 14, lineHeight: 1.45, marginBottom: 8 }}>
                            <span style={{ color: 'var(--ok)', flexShrink: 0 }}>+</span><span>{s}</span>
                          </div>
                        ))}
                      </div>
                      <div>
                        ...
                        {c.gaps.map((g, j) => (
                          <div key={j} style={{ display: 'flex', gap: 10, fontSize: 14, lineHeight: 1.45, marginBottom: 8 }}>
                            <span style={{ color: 'var(--accent)', flexShrink: 0 }}>–</span><span>{g}</span>
                          </div>
                        ))}
      ...
      <div className="stage">
        <div className="footer">
          <div className="row">
            <div>{H.id.toUpperCase()} · DESTINATION HUB · PHÂN TÍCH THỊ TRƯỜNG</div>
...
export function DestinationVariantDetail({ hub: H, id, go }) {
  ...
  if (!v) return (
    <div className="page-anim">
      <section className="section"><div className="stage">
        ...
        <button onClick={() => go({ tab: 'tours', id: H.id })} style={{ marginTop: 20, fontFamily: 'var(--mono)', fontSize: 11, padding: '10px 18px', background: 'var(--ink)', color: 'var(--paper)', border: 'none', cursor: 'pointer' }}>← Về {H.name} Hub</button>
      </div></section>
    </div>
  );
    ...
    <div className="page-anim">
      ...
      <section className="section" style={{ paddingBlock: '48px 48px' }}>
        <div className="stage">
          <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
            {v.isFlagship && <span style={{ background: 'var(--accent)', color: 'var(--paper)', fontSize: 10, padding: '3px 8px', fontFamily: 'var(--mono)', letterSpacing: '0.1em' }}>#1 FLAGSHIP {H.name.toUpperCase()}</span>}
          ...
          <div className="stage">
            <div style={{ padding: '20px 24px', borderLeft: '3px solid var(--ink-3)', background: 'var(--paper-3)' }}>
              ...
              <div style={{ fontFamily: 'var(--sans)', fontSize: 15, lineHeight: 1.6, color: 'var(--ink-2)' }}>
                {v.skeletonNote || 'Skeleton SKU — thin volume + fragmented ops. Monitor only.'}
              </div>
            </div>
          </div>
        </section>
              ...
              <div className="grid-2" style={{ gridTemplateColumns: '280px 1fr', gap: 64 }}>
                ...
                <div>
                  ...
                  {v.seasonalityFootnote && (
                    <div style={{ marginTop: 12, fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--ink-3)', letterSpacing: '0.06em' }}>{v.seasonalityFootnote}</div>
                  )}
                </div>
              </div>
            </div>
      ...
      {v.history && (() => {
        ...
        return (
          <section className="section">
            <div className="stage">
              <SectionHeader kicker="Xu hướng sản phẩm qua các năm" title={`Đà tăng trưởng ${v.history[0].year} → ${all[all.length - 1].year}F.`} dek="Bookings/năm + driver từng năm. Cột đậm = actual, cột đứt = forecast." />
              {v.whyHot && (
                <div style={{ borderTop: '2px solid var(--ink)', paddingTop: 24, paddingBottom: 24, marginBottom: 24, borderBottom: '1px solid var(--rule)' }}>
                  <div className="h-kicker" style={{ marginBottom: 12 }}>Vì sao sản phẩm này đang hot — {v.whyHot.length} drivers</div>
              ...
              <div style={{ borderTop: v.whyHot ? 'none' : '2px solid var(--ink)', paddingTop: v.whyHot ? 0 : 32 }}>
                  ...
                  {all.map((y, i) => {
                    ...
                    return (
                      <div key={y.year} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                        <div style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 500 }}>{(y.bookings / 1000).toFixed(0)}K</div>
                        <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: y.growth && y.growth.startsWith('+') ? (isForecast ? 'var(--ink-3)' : 'var(--ok)') : y.growth && y.growth.startsWith('-') ? 'var(--accent)' : 'var(--ink-4)', fontWeight: 500, minHeight: 14 }}>{y.growth || '—'}</div>
                        <div style={{ width: '100%', height: Math.round(pct * 140) + 'px', background: isForecast ? 'var(--paper-3)' : 'var(--accent)', border: isForecast ? '2px dashed var(--accent)' : 'none' }} />
                      </div>
                    );
                  })}
                </div>
                <div style={{ display: 'flex', gap: 16, marginBottom: 32, paddingInline: 20 }}>
                  {all.map((y, i) => (
                    <div key={y.year} style={{ flex: 1, textAlign: 'center', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 500, color: i >= v.history.length ? 'var(--ink-3)' : 'var(--ink)' }}>
                      {y.year}{i >= v.history.length ? 'F' : ''}
                    </div>
                  ))}
                </div>
                  ...
                  {all.map((y, i) => {
                    ...
                    return (
                      <div key={y.year} style={{ display: 'grid', gridTemplateColumns: '80px 130px 1fr', gap: 20, padding: '16px 0', borderBottom: '1px solid var(--rule)', alignItems: 'start' }}>
                        <div style={{ fontFamily: 'var(--mono)', fontSize: 14, fontWeight: 500, color: isForecast ? 'var(--ink-3)' : 'var(--ink)' }}>{y.year}{isForecast ? 'F' : ''}</div>
                        <div>
      ...
      {v.trends && (
        <section className="section" style={{ background: 'var(--paper-2)' }}>
          <div className="stage">
            <SectionHeader kicker={`${v.trends.length} xu hướng định hình 2026–2028`} title="Sau đường forecast — cái nào đi sớm thì thắng." dek="Xu hướng macro thị trường cho phân khúc này." />
            <div style={{ borderTop: '2px solid var(--ink)' }}>
              {v.trends.map((t, i) => (
        ...
        <section className="section" style={{ background: 'var(--paper-2)' }}>
          <div className="stage">
            ...
            <div style={{ borderTop: '2px solid var(--ink)', paddingTop: 24 }}>
              {vHasDays ? (
                [...new Set(v.itinerary.map((s) => s.day))].map((d) => (
              ...
              {v.itineraryNote && (
                <div style={{ marginTop: 16, fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--ink-3)', lineHeight: 1.6 }}>⚠ {v.itineraryNote}</div>
              )}
            </div>
      ...
      {v.topCompetitors && (
        <section className="section">
          <div className="stage">
            <SectionHeader kicker={`Top ${v.topCompetitors.length} đối thủ cạnh tranh`} title="Ai đang chạy sản phẩm này — mạnh ở đâu, hở ở đâu." />
            <div style={{ borderTop: '2px solid var(--ink)' }}>
        ...
        <section className="section" style={{ background: 'var(--paper-2)' }}>
          <div className="stage">
            <SectionHeader
              ...
              dek="Pain points mined từ TripAdvisor + operator reviews 2023–2025."
            />
            {v.painPoints && (
              <div style={{ borderTop: '2px solid var(--ink)' }}>
                  ...
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '32px 1fr 140px 80px 1.2fr', gap: 20, padding: '22px 0', borderBottom: '1px solid var(--rule)', alignItems: 'start' }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 3 }}>
                        {[1,2,3,4,5].map((s) => <span key={s} style={{ color: s <= p.sev ? 'var(--accent)' : 'var(--rule)', fontSize: 12 }}>★</span>)}
                      </div>
                    </div>
                    <div><div className="label" style={{ marginBottom: 2 }}>Tần suất</div><div className="num" style={{ fontSize: 13, color: 'var(--accent)' }}>{p.freq}</div></div>
                    <div><div className="label" style={{ marginBottom: 2 }}>Độ nặng</div><div className="num" style={{ fontSize: 16, color: p.sev >= 5 ? 'var(--accent)' : 'var(--ink-2)' }}>{p.sev}/5</div></div>
            ...
            {v.reviewSnippets && (
              <div style={{ marginTop: 40, paddingTop: 32, borderTop: '2px solid var(--ink)' }}>
                <div className="h-kicker" style={{ marginBottom: 16 }}>Quote khách thật</div>
                <div className="grid-2" style={{ gridTemplateColumns: '1fr 1fr', gap: 40 }}>
                  {['low', 'high'].map((tier) => (
                    <div key={tier}>
                      <div className="h-kicker" style={{ color: tier === 'low' ? 'var(--accent)' : 'var(--ok)', marginBottom: 12 }}>
                        {tier === 'low' ? '★ 1–4 · Pain driver' : '★ 5 · What works'}
                      </div>
                      {v.reviewSnippets[tier].map((r, i) => (
                        <div key={i} style={{ marginBottom: 14, padding: '14px 18px', background: 'var(--paper)', borderLeft: `3px solid ${tier === 'low' ? 'var(--accent)' : 'var(--ok)'}` }}>
                          <div style={{ display: 'flex', gap: 2, marginBottom: 6 }}>
                            {[1,2,3,4,5].map((s) => <span key={s} style={{ color: s <= r.star ? (tier === 'low' ? 'var(--accent)' : 'var(--ok)') : 'var(--rule)', fontSize: 11 }}>★</span>)}
                          </div>
      ...
      {v.riskFactors && (
        <section className="section" style={{ background: 'var(--paper-2)' }}>
          <div className="stage">
            <SectionHeader kicker={`${v.riskFactors.length} rủi ro chính`} title="Điều gì có thể làm sản phẩm này flop." dek="Downside scenario + likelihood + mitigation." />
            <div style={{ borderTop: '2px solid var(--ink)' }}>
              {v.riskFactors.map((r, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '36px 1fr 80px 1fr 1fr', gap: 20, padding: '24px 0', borderBottom: '1px solid var(--rule)', alignItems: 'start' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 3 }}>
                      {[1,2,3,4,5].map((s) => <span key={s} style={{ color: s <= r.severity ? 'var(--accent)' : 'var(--rule)', fontSize: 12 }}>●</span>)}
                    </div>
                  </div>
                  <div><div className="label" style={{ marginBottom: 2 }}>Severity</div><div className="num" style={{ fontSize: 15, color: r.severity >= 4 ? 'var(--accent)' : 'var(--ink-2)' }}>{r.severity}/5</div></div>
                  <div style={{ fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.55 }}><div className="label" style={{ marginBottom: 4 }}>Likelihood</div>{r.likelihood}</div>
                  <div className="callout" style={{ margin: 0, borderLeftColor: 'var(--ok)' }}>
      ...
      <section className="section">
        <div className="stage">
          <button
            onClick={() => go({ tab: 'tours', id: H.id })}
            style={{ fontFamily: 'var(--mono)', fontSize: 11, padding: '12px 20px', background: 'var(--paper)', color: 'var(--ink)', border: '1px solid var(--ink)', cursor: 'pointer', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
            ← Quay lại {H.name} Hub
          </button>
        </div>
      </section>
      ...
      <div className="stage">
        <div className="footer">
          <div className="row">
            <div>{H.id.toUpperCase()} · {v.id.toUpperCase()} · PHÂN TÍCH SẢN PHẨM</div>
